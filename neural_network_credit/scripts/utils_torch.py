...
class twolayers_NN(nn.Module):
    ...
    def __init__(self, epsilon, mode):
        ...
        if mode == 'case1':
            ...
            self.fc2 = nn.Linear(64, 1, dtype = torch.float64)
...
def evaluate_logistic_loss(X, Y, l2_penalty, model=None, eval_bool=False, mode='RS', theta=None):
    """Compute the l2-penalized loss."""

    if mode == 'default':
        n = X.shape[0]
        logits = X @ theta
        log_likelihood = (1.0 / n) * np.sum(-1.0 * np.multiply(Y, logits) + np.log(1 + np.exp(logits)))
        ...
        reg = (l2_penalty / 2.0) * _model_l2_penalty(model)
        return (1.0 / n) * torch.sum(loss) + reg
